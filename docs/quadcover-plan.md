...
**M2 — Port the isoline extractor — ✅ DONE.** Ported `PositionKey`/`Double`/`MeshSeparator`/`extractConnections`/`extractEdges`/`simplifyGraph`/`collapse*`/`extractMesh`/`fixHoles`/`removeIsolated`/`NonManifoldFaces` (~1240 LOC) to our `Vec3`/`Mesh`; wired `extractIsolineQuads`. **Boundary-aware gate:** the closed-surface graph cleanup (simplifyGraph/fixHoles/collapse*) is REQUIRED on a closed surface but CORRUPTS an open disk, so `extractIsolineQuads` runs it iff the isotropic mesh is closed (`isClosedTriMesh`). *Checkpoints (both green):* flat integer-grid UV → exact NxN quad grid, 0 interior irregular (open→core); UV sphere via the M1 harness → **2504 quads, 2 non-quad, manifold, 1.1% interior irregular** (closed→cleanup) — QuadriFlow-class (1–4%). Locked by `test_quadcover_extractor.cpp` (flat-grid checkpoint always; sphere assertions run when `CYBER_QUADCOVER_CLI` is set).

**M3 — Full IQuadrangulator + CAPI — ✅ DONE.** `QuadCoverQuadrangulator::quadrangulate()` runs M1 (computeSeamlessUv) → M2 (extractIsolineQuads) → in-place `Mesh::fromIndexed` rewrite, failing cleanly (mesh untouched) when the harness is absent. Added `CYBER_QUAD_QUADCOVER = 3` (capi.h + capi.cpp dispatch), `quad_method="quad-cover"` (python api). **Pipeline fix:** the method sets `quadCoverMethod` in `pipeline.cpp`, which SKIPS our isotropic remesh stage — the harness does its own, and double-remeshing left it a density-sensitive, crack-prone input that broke extraction. **Gate hardening:** replaced the brittle binary closed-test with `boundaryEdgeFraction < 0.10` (a harness re-remesh leaves a few numerical cracks that flip a truly-closed model to "open"). **Result (11_benchmark.py, spot@3000):** ours quad-cover **irr 3% — TIES QuadriFlow (3%)** and beats field-aligned (9%), integer (7%), AutoRemesher (6%); also best median angle (83°) and normal error (5.9°). First QuadriFlow-parity on the irregular metric. Open items → M4: quad count overshoots target (harness `-f`/`-s` density mapping), and edge CV (0.41) is worse than QF (0.14).

**M4a — Count calibration — ✅ DONE.** The extracted quad count now tracks the requested target. Root cause of the old overshoot: the harness `-s` scaling only loosely maps to the extracted count (asked 650, got 1179 ≈ 1.8×), and the pure-quads path then 4×-subdivides a coarse `target/4` base — compounding to 4758 vs 3000. Fix: `computeSeamlessUv` takes a `harnessScaling`, and `QuadCoverQuadrangulator` runs a short closed loop (extract → measure vs `area/edge²` → correct scaling by `sqrt(actual/target)`, one retry if off >25%). Spot@3000 pure-quads: **4758 → 2776** (within 8%); tracks across 1500/3000/6000.
...
quad-cover now **beats QuadriFlow on irregular for spot + rocker-arm**, is within 1–2 pts on fandisk/cheburashka, ties on CV, and has best-or-tied median angle everywhere — while crushing our field-aligned (9–16%) and integer (7–9%). This is genuine QuadriFlow parity, honestly measured at matched density; the earlier density-artifact caveat no longer applies. **Remaining:** adaptive-sizing quad-cover (curvature `-a`>0 with reprojection) is future work.

**Adaptive-sizing quad-cover — implemented, but measured COUNTERPRODUCTIVE (kept uniform-only).** Plumbed the run's adaptivity through to the harness `-a` (`computeSeamlessUv(..., harnessAdaptivity)`, `makeQuadCoverQuadrangulator(iters, adaptivity)`). Measured at matched density (adaptivity 0→1): surface deviation is flat-to-worse (spot 0.258→0.248%, fandisk 0.293→0.296%, cheb 0.278→0.317%) while irregular and CV blow up (spot irr 2→6% / CV 0.15→0.44; fandisk 3→15% / 0.20→0.51; cheb 4→14% / 0.28→0.59). Unlike the field/integer paths (where adaptivity lowers per-polygon deviation), the seamless-UV solve's strength IS its uniform global grid — the frame field already follows curvature by direction, so adding scaling gradients only injects singularities with no fidelity payoff. So quad-cover stays **uniform-only** in the default path (the CAPI forwards adaptivity 0), matching QuadriFlow/AutoRemesher which are also uniform-only. The knob is retained for experiments (`makeQuadCoverQuadrangulator(iters, a)` / `CYBER_QC_ADAPT`).

**M4c — In-process solver (no subprocess / no `CYBER_QUADCOVER_CLI`) — ✅ DONE.** The seamless-UV solve now runs **in-process**: a new isolated static library `cyber_quadcover_solver` (`cmake/QuadCoverSolver.cmake`, built only when `-DCYBER_WITH_QUADCOVER=ON`) compiles the same Geogram 1.8.3 + AutoRemesher + isotropicremesher subset the harness used (parsed from `autoremesher.pro`, kept in lockstep with `build_autoremesher.sh`) plus `examples/reference/autoremesher_solve.{hpp,cpp}`, which exposes `cyber::remesh::qcsolver::solveSeamlessUv()` over plain std types (no Geogram types leak into the C++20 tree). The target is isolated with `-std=c++14 -w -DGEO_STATIC_LIBS` and never calls `cyber_set_warnings`; it links `OpenMP tbb z dl m`. When `CYBER_WITH_QUADCOVER=ON`, `cyber_quadrangulate` links it and defines `CYBER_HAVE_QUADCOVER`, and `computeSeamlessUv` takes the in-process branch (mesh→verts/tris, same target-quad/scaling math, `CYBER_QC_SCALING`/`CYBER_QC_ADAPT` still honored, adaptivity default `0.0`); the subprocess path is retained as the `#else` fallback. **Teardown:** `GEO::initialize()` runs once via `std::call_once` and `GEO::terminate()` is never called — the process exits cleanly (the harness's `std::_Exit(0)` double-free did **not** materialize in-process; full suite exits 0). **Result:** the M1/M3 sphere tests now pass with `CYBER_QUADCOVER_CLI` **unset** (`uv.valid == true`, residual ~0, ~1% interior irregular); full suite **241/241** with the option ON and OFF. Build: `cmake --preset cpu-headless -DCYBER_WITH_QUADCOVER=ON && cmake --build --preset cpu-headless`.
