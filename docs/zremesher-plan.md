...
| **F** | semantic boundaries; exact forced-axis topology symmetry | mirrored *connectivity*, not merely mirrored positions — **MET** for forced symmetry (0 unmatched vertices and faces on every corpus model). Semantic boundaries are **MET** for typed face-domain `group_id` / `material_id`: coplanar mismatches become hard features and per-component final-mesh evidence is returned. Automatic detection remains unstarted. |
